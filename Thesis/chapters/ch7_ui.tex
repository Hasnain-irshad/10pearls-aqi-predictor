...
system it uses, and each of its five views. The figures in this chapter are
...
\textbf{Publish the system's own record.} Four of the five tabs are given over
...
\section{The Analytics and SHAP View}
The Analytics view makes the model and its data inspectable without requiring a
reader to open the notebook. Its first panel ranks the fifteen strongest global
features by mean absolute SHAP impact. This answers the question ``what does the
model rely on overall?'' and complements the per-city explanation card on the
Forecast view, which answers ``why this forecast?'' for one selected city and peak
hour.

The remaining panels provide statistical context: a historical AQI distribution
histogram with EPA thresholds, an AQI category breakdown, a city-average ranking,
seasonal and diurnal patterns, and a Pearson correlation heatmap covering AQI and
the principal weather variables. A compact summary row surfaces the observation
count, strongest SHAP driver, highest-risk city and strongest feature relationship
before the detailed charts. These views are backed by committed JSON snapshots in
static mode and by the FastAPI statistics and SHAP endpoints in dynamic mode; the
frontend falls back to the snapshots if an analytics endpoint is temporarily
unavailable.
...
behind it, and its five views. The Forecast view answers the user's question

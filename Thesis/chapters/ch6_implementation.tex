...
recovery strategy for every failure in this system to be ``run it again''. After
the feature-store write, the hourly GitHub Actions workflow also runs batch
inference, commits the refreshed \texttt{predictions.json}, and pushes it to the
repository. This publication step makes the hourly observation refresh visible
to the public dashboard rather than leaving the website dependent on the separate
daily training job.
...
back to the repository. Only the training step can fail the workflow; the rest
run unconditionally. That is a deliberate consequence of principle P2: inference
reads the provider and the model registry but never the feature store, so a
feature-store outage during training must not also cost the site its daily
forecast refresh. Evaluation and monitoring, which do read the feature store, are
allowed to fail quietly for the same reason. The commit message carries a skip marker so that committing artefacts
...
The nightly training run failed on five of ten nights & The free-tier Arrow
Flight query service intermittently drops a large read, surfacing as
\emph{FlightUnavailableError: Socket closed} with gRPC status 14. It is not
deterministic: the identical read succeeds on other nights. & Retry the read with
increasing backoff, and make inference, evaluation, monitoring and the commit run
whether or not training succeeded, so a feature-store outage costs a retrain but
not the day's forecast. \\
